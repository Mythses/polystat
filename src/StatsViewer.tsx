...
const StatsViewer = () => {
    ...
    const fetchStats = useCallback(async (page = 1) => {
        if (!userIdState) { setError('Please enter a User ID.'); return; }
        ...
        try {
            ...
            const leaderboardUrl = `${PROXY_URL}${encodeURIComponent(API_BASE_URL + `?version=${VERSION}&trackId=${trackId}&skip=${skip}&amount=${AMOUNT}&onlyVerified=${onlyVerified}&userTokenHash=${userIdState}`)}`;
            ...
            if (userEntry) {
                ...
                const userAmount = 20;
                const userUrl = `${PROXY_URL}${encodeURIComponent(API_BASE_URL + `?version=${VERSION}&trackId=${trackId}&skip=${userSkip}&amount=${userAmount}&onlyVerified=${onlyVerified}&userTokenHash=${userIdState}`)}`;
                const userResponse = await fetch(userUrl);
                if (!userResponse.ok) {
                    throw new Error(`Failed to fetch user data: ${userResponse.status}`);
                }
                const uData: LeaderboardData = await userResponse.json();
                const foundUser = uData.entries.find((e) => e.userId === userIdState);
                if (foundUser) {
    ...
    }, [userIdState, trackId, onlyVerified]);

    useEffect(() => { if (trackId && userIdState) fetchStats(1); }, [trackId, userIdState, fetchStats]);
                                    ...
                                    <CardHeader>
                                        <CardTitle className="text-2xl sm:text-3xl font-bold text-purple-400 text-center flex items-center justify-center gap-2 flex-wrap">
                                            {getPosMedal(userData.rank) ? (
                                                <>
                                                    <span
                                                        data-tooltip-id="statsMedal"
                                                        data-tooltip-content={getPosMedal(userData.rank)?.label}
                                                        style={{ color: getPosMedal(userData.rank)?.color }}
                                                    >
                                            ...
                                            {getMedal(userData.percent, userData.position) ? (
                                                <>
                                                    <span
                                                        data-tooltip-id="statsMedal"
                                                        data-tooltip-content={getMedal(userData.percent, userData.position)?.label}
                                                        style={{ color: getMedal(userData.percent, userData.position)?.color }}
                                                    >
                                                        {getMedal(userData.percent, userData.position)?.icon}
                                                    </span>
                                                    <div id="statsMedal" className="rounded-md" style={{ backgroundColor: "rgb(27, 21, 49)", color: getMedal(userData.percent, userData.position)?.color, fontSize: "1rem", padding: "0.25rem 0.5rem" }}></div>
                                                </>
                                            ) : null}
                                        </CardTitle>
                                                ...
                                                <div key={entry.id} className={cn(
                                                    ...
                                                    <div className="grid grid-cols-1 gap-2">
                                                        ...
                                                        <div><span className="font-semibold text-gray-300">Top:</span> <span className={userTextStyle}>{entry.percent ? entry.percent.toFixed(4) : 'N/A'}%</span>
                                                            ...
                                                            <div id={`medal-${entry.id}`} className="rounded-md" style={{ backgroundColor: "rgb(27, 21, 49)", color: medal?.color, fontSize: "1rem", padding: "0.25rem 0.5rem" }}></div>
                                                            <div id={`posMedal-${entry.id}`} className="rounded-md" style={{ backgroundColor: "rgb(27, 21, 49)", color: posMedal?.color, fontSize: "1rem", padding: "0.25rem 0.5rem" }}></div>
                                                        </div>
